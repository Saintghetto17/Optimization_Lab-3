...
def task(optimizer, epoch, batch, X_train, X_test, Y_train, Y_test):
    model = Sequential()
...
if __name__ == "__main__":
    ...
    optimizers_configuration = {"SGD": {"learning_rate": [0.01, 0.001, 0.0001, 0.00001],
                                        "clipnorm": [1]},
                                "Nesterov": {"learning_rate": [0.01, 0.001, 0.0001, 0.00001],
                                             "clipnorm": [1],
                                             "nesterov": [True]},
                                "Momentum": {"learning_rate": [0.01, 0.001, 0.0001, 0.00001],
                                             "clipnorm": [1],
                                             "momentum": [0.1, 0.25, 0.5, 0.75, 0.9, 0.999]},
                                "Adam": {"learning_rate": [0.01, 0.001, 0.0001, 0.00001],
                                         "beta_1": [0.9, 0.85, 0.8, 0.75, 0.5],
                                         "beta_2": [0.999, 0.999999, 0.95, 0.9],
                                         "amsgrad": [False, True]},
                                "RMSprop": {"learning_rate": [0.01, 0.001, 0.0001, 0.00001],
                                            "rho": [0.9, 0.75, 0.5, 0.25, 0.1],
                                            "centered": [False, True]},
                                "Adagrad": {"learning_rate": [0.01, 0.001, 0.0001, 0.00001],
                                            "initial_accumulator_value": [0.1, 0.05, 0.25]}}
    batches = [64, 128, 256, 512]
    epochs = [8, 64, 512]
    # epochs = [8, 64, 512, 4096]
    try:
        os.mkdir("results")
        ...
        for optimizer_name, optimizer_raw in optimizers.items():
            ...
            for epoch in epochs:
                ...
                for configuration in conf_combinations:
                    print(
                        f"Optimizer: {optimizer_name}, Epochs: {epoch}, Batch: {batch}, Configuration: {configuration}")
                    optimizer = optimizer_raw(**configuration)
                    args = (optimizer, epoch, batch, X_train, X_test, Y_train, Y_test)
                    kwargs = {}
                    result, memory_usage, elapsed_time = measure_resources(task, *args, **kwargs)
                    if result < min_loss:
                        min_loss = result
                ...
                overall_results[epoch][optimizer_name] = (min_loss, min_loss_config, batch)
    ...
    for epochs, data in overall_results.items():
        ...
        for key in epo_min_keys:
            print(f"  Optimizer: {key}, Loss: {data[key][0]}, BatchSize {data[key][2]}, Configuration:{data[key][1]}")
